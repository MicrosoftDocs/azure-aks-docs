...
* The add-on doesn't support automatic upgrades. It can be enabled on [AKS Automatic][aks-automatic] clusters, but it isn't upgraded along with the cluster, so you need to [upgrade the add-on][istio-upgrade] to ensure the mesh revision remains compatible with the cluster's Kubernetes version.
