...
* The add-on doesn't support automatic upgrades. It can be enabled on [AKS Automatic][aks-automatic] clusters, but it isn't upgraded when the cluster auto-upgrades, so an out-of-date mesh revision can eventually block the cluster's automatic upgrade. [Upgrade the add-on][istio-upgrade] to keep it compatible with the cluster's Kubernetes version.
