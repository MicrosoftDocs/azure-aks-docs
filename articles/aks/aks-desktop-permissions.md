---
title: Get started with AKS desktop (Preview)
...
# Get started with AKS desktop (Preview)

AKS desktop builds on existing AKS and Azure features to provide an application-focused experience for deploying and managing workloads. This article helps you get started with AKS desktop by explaining the required infrastructure and permissions for two common scenarios: cluster operators who set up infrastructure and developers who deploy applications.

> [!NOTE]
> AKS desktop is in early stages of public preview. During the public preview, AKS desktop might undergo design changes, add/delete additional features, and more. If you're interested in shaping the AKS desktop experience, engage with the engineers and product team at the official [AKS desktop GitHub repository](https://github.com/Azure/aks-desktop).

## Prerequisites for all users

* An Azure subscription. If you don't have an Azure subscription, you can create a [free account](https://azure.microsoft.com/free).
* Azure CLI version 2.64.0 or later installed and configured. Run `az --version` to find the version. If you need to install or upgrade, see [Install Azure CLI](/cli/azure/install-azure-cli).
...
To use AKS desktop, you need the following Azure resources:

* **Resource group** - Contains your AKS cluster and any AKS managed projects created through AKS desktop.
* **AKS Automatic cluster** - The Kubernetes cluster where your applications run.
* **Azure Container Registry (ACR)** - Stores your container images for deployment.
...
* **[Scenario 1: I'm a cluster operator](#scenario-1-cluster-operator-set-up-infrastructure)** - You're responsible for setting up the AKS infrastructure, creating clusters, and managing access for development teams.

* **[Scenario 2: I'm a developer](#scenario-2-developer-deploy-and-manage-applications)** - You need to deploy applications, manage projects, and view application metrics in an existing AKS desktop environment.

---

## Scenario 1: Cluster operator: Set up infrastructure

As a cluster operator, you're responsible for creating and configuring the infrastructure that development teams use. This includes setting up the AKS cluster, configuring Azure Container Registry access, and managing permissions for project creators.

### What you'll do

1. Create the required Azure infrastructure (resource group, AKS cluster, ACR)
2. Configure ACR integration with your AKS cluster
3. Assign permissions for users to create projects
4. Optionally, delegate permission management to project creators

### Step 1: Create infrastructure resources

To create the infrastructure resources, you need permissions to create resources in Azure. If you don't have the **Owner** RBAC role, you need the **Contributor** role to create resources and the **User Access Administrator** role to assign permissions to other users.

#### Assign permissions to create resources

1. Set environment variables for your resource group and cluster name.
...
2. Assign the necessary role to create resources in the resource group using the [`az role assignment create`](/cli/azure/role/assignment#az-role-assignment-create) command.
...
Assign the role using the [`az role assignment create`](/cli/azure/role/assignment#az-role-assignment-create) command:
...
## Scenario 2: Developer: Deploy and manage applications

As a developer, you work within an existing AKS desktop environment to deploy applications, manage projects, and monitor your workloads. The cluster operator grants you the necessary permissions to perform these tasks.

### What you'll do

1. Deploy applications into projects
2. View and manage deployed applications
3. Monitor application metrics and logs
4. Modify project access (if needed)

### Understanding automatic permissions
...
* **Shared access users** - Users granted access during project creation receive:
  ...
  * One of the following Kubernetes RBAC roles:
...
To view metrics, you need the **Monitoring Data Reader** role on the Azure Monitor workspace. Note that this role grants access to all metrics for the cluster, not just your specific projects.
...
2. Assign the **Monitoring Data Reader** role using the [`az role assignment create`](/cli/azure/role/assignment#az-role-assignment-create) command.
...
Currently, AKS desktop doesn't provide a UI option to modify project permissions after creation. If you need to update your permissions or grant access to others, work with your cluster operator to update permissions using the Azure portal or Azure CLI.

The process involves three steps:

#### Step 1: Grant access to the managed namespace resource
...
#### Step 2: Assign Kubernetes RBAC role
...
#### Step 3: Grant permissions to view metrics

Follow the steps in the [View application metrics](#view-application-metrics) section to grant users access to view application metrics.
...
* Review [Azure built-in roles](/azure/role-based-access-control/built-in-roles)
