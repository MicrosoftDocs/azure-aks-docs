...
1. Grant the cluster's control plane managed identity **Reader** access on the resource group containing the PIS. The AKS control plane checks `Microsoft.ContainerService/preparedImageSpecifications/read` on the PIS version resource before the node pool can reference it. Use the [`az role assignment create`][az-role-assignment-create] command.
...
- The cluster's control plane managed identity has the **Reader** role on the resource group containing the PIS. See [Create a node pool using a PIS](#create-a-node-pool-using-a-pis).
