...
1. Create a node pool using the [`az aks nodepool add`][az-aks-nodepool-add] command and set `--gpu-driver` field to `none` to skip default GPU driver installation.
   ...
   Note: if you get the error "unrecognized arguments: --gpu-driver none", [update the Azure CLI version](https://learn.microsoft.com/cli/azure/update-azure-cli?view=azure-cli-latest). 
