...
* In scenarios where the VNet containing your cluster has custom DNS settings, cluster deployment fails unless the private DNS zone is linked to the VNet that contains the custom DNS resolvers. Even when using a bring-your-own private DNS zone already linked to a hub VNet with custom resolvers, AKS will still attempt—by design—to link the zone to the cluster’s VNet during provisioning. If the cluster’s managed identity lacks sufficient permissions on the VNet (e.g., Contributor at the VNet scope), the deployment may fail. This link may be functionally redundant in hub-and-spoke DNS setups but is still required for successful provisioning.
