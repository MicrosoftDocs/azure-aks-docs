...
| Application insights | [Azure Monitor Application Insights](/azure/azure-monitor/app/app-insights-overview) is an Application Performance Management (APM) tool for monitoring application-level signals. It collects logs, metrics, and distributed traces for analysis in the Azure portal. To enable Application Insights, see [data collection basics](/azure/azure-monitor/app/opentelemetry-overview). For autoinstrumentation, which monitors applications without source code changes, see [Application monitoring for Kubernetes](/azure/azure-monitor/app/kubernetes-codeless). |
