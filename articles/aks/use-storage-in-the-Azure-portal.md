---
title: Use storage in the Azure portal
description: In this article, you learn how to create and manage storage resources for AKS clusters in the Azure portal.
author: sasper0ni
ms.author: mirandawu
...
---
# Use storage in the Azure portal
...
Once you've created or selected a storage class, you can create a PVC that will reference it to dynamically provision a storage resource. Update the following fields under the **Persistent Volume Claim details** section and once you're done, select **Create**.
...
> [!TIP]
> Shrinking persistent volumes is not currently supported, so be sure to size your PVC accordingly. For certain Azure Disk SKUs, the size designated here may correspond to your storage's performance tier. [Learn more about Azure Disk tiers](/azure/virtual-machines/disks-types)
...
The example YAML manifest shown below creates a pod that runs an NGINX image and mounts your storage resource at the _/mnt/azure_ path. For Windows Server containers, specify a `mountPath` using the Windows path convention, such as _'D:'_.
...
Currently, the Portal only explicitly supports storage creation via dynamic provisioning (using storage classes and PVCs). To statically provision a PV, you can manually apply a YAML definition via **+ Create** > **Apply a YAML** in the **Storage** menu.
