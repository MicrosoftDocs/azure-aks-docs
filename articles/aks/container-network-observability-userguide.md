...
L7 Flows (Namespace): shows http, Kafka and gRPC packet flows to/from the specified namespace (i.e. Pods in the Namespace) when a Layer 7 based policy is applied.

L7 Flows (Workload): shows http, Kafka and gRPC flows to/from the specified workload (e.g. Pods of a Deployment or DaemonSet) when a Layer 7 based policy is applied.

## Use Case 1. Demystifying Pod-Level DNS Issues in Kubernetes and Leveraging ACNS for Root Cause Analysis (RCA)
...
Let us assume that user has set up a web application in AKS cluster and now that web application is not reachable. One common issue could be the DNS server is throwing an error while resolving it.
...
- Starting point for user would be to check DNS cluster dashboard to get snapshot of all DNS activities. It gives high level overview of DNS requests and responses. Below dashboard shows - what kinds of queries lack responses, what is the most common query and most common response Top DNS errors and nodes that creates most of those errors. Refer following image for that glimpse.

[![Snapshotof DNS cluster](./media/advanced-container-networking-services/ACNSdashboard/DNSClustersnapshotcombined.png)](./media/advanced-container-networking-services/ACNSdashboard/DNSClustersnapshotcombined.png#lightbox)

- User can also scroll down to find out pods with most DNS request and errors in all namespaces.
[![Snapshots of top pods in all namespaces](./media/advanced-container-networking-services/ACNSdashboard/TopPodswithDNSerrorsinworkload.png)](./media/advanced-container-networking-services/ACNSdashboard/TopPodswithDNSerrorsinworkload.png#lightbox)
...
[![Snapshots of DNS requests and responses in specific workload](./media/advanced-container-networking-services/ACNSdashboard/WorkloadDNSRequestsandresponses4.png)](./media/advanced-container-networking-services/ACNSdashboard/WorkloadDNSRequestsandresponses4.png#lightbox)

- Next step is to check for any DNS errors by type. Check for spikes in specific error types (e.g., NXDOMAIN for non-existent domains). In this example, there is a significant increase in Query refused errors, suggesting a mismatch in DNS configurations or unsupported queries.

[![Snapshots of DNS Errors by type](./media/advanced-container-networking-services/ACNSdashboard/DNSerrorwithtype5.png)](./media/advanced-container-networking-services/ACNSdashboard/DNSerrorwithtype5.png#lightbox)

- Use panels like "DNS Response IPs Returned" to ensure that expected responses are being processed. This graph displays the rate of successful DNS queries processed per second. This information is useful for understanding how frequently DNS queries are being successfully resolved for the specified workload.
    - Increased rate: Could indicate a surge in traffic or a potential DNS attack (e.g., DDoS).
    - Decreased rate: Might signify issues with DNS servers, workload connectivity, or configuration errors.

[![Snapshots of DNS Responses IP Returned](./media/advanced-container-networking-services/ACNSdashboard/DNSresponsesIPreturned6.png)](./media/advanced-container-networking-services/ACNSdashboard/DNSresponsesIPreturned6.png#lightbox)

- Examining the most frequent DNS queries can help identify patterns in network traffic. This information is useful for understanding workload distribution and detecting any unusual or unexpected query behaviors that may require attention.

[![Snapshots of top DNS queries](./media/advanced-container-networking-services/ACNSdashboard/TopDNSqueries7.png)](./media/advanced-container-networking-services/ACNSdashboard/TopDNSqueries7.png#lightbox)

- The following table aids DNS issue root cause analysis by highlighting query types, responses, and error codes like "non-existent domain." It identifies problematic queries, patterns of failures, or misconfigurations. By observing trends in return codes and response rates, users can pinpoint specific nodes, workloads, or queries causing DNS disruptions or anomalies. For example, users can see that for Quad-A records, there is no error, but for A record example.com, there is a server failure.

[![Snapshots of DNS response table](./media/advanced-container-networking-services/ACNSdashboard/DNSresponsetable8.png)](./media/advanced-container-networking-services/ACNSdashboard/DNSresponsetable8.png#lightbox)
...
[![Snapshots of pods with most DNS errors](./media/advanced-container-networking-services/ACNSdashboard/TopPodswithDNSerrorsinworkload9.png)](./media/advanced-container-networking-services/ACNSdashboard/TopPodswithDNSerrorsinworkload9.png#lightbox)

### Step 2- > Debugging DNS Resolution of a Pod with flow logs
...
[![Snapshots of Hubble Cli logs ](./media/advanced-container-networking-services/ACNSdashboard/hubblednsresponsecombined10.png)](./media/advanced-container-networking-services/ACNSdashboard/hubblednsresponsecombined10.png#lightbox)
...
[![Snapshots of Hubble Cli logs ](./media/advanced-container-networking-services/ACNSdashboard/hubbleuiservicemap11.png)](./media/advanced-container-networking-services/ACNSdashboard/hubbleuiservicemap11.png#lightbox)
...
[![Snapshots of Drop metrics ](./media/advanced-container-networking-services/ACNSdashboard/Workloadsnapshotwithjustmetrics14.png)](./media/advanced-container-networking-services/ACNSdashboard/Workloadsnapshotwithjustmetrics14.png#lightbox)

- Examine Dropped Traffic by Reason: Use the Dropped Incoming/Outgoing Traffic by Reason graph to identify the root cause of the drops. In this case, the reason is policy denied, indicating misconfigured network policies blocking outgoing traffic. Check if any specific time interval shows a spike in drops to narrow down when the issue started.

[![Snapshots of Droppedtraffic withreasons ](./media/advanced-container-networking-services/ACNSdashboard/Droppedtrafficbyreason15.png)](./media/advanced-container-networking-services/ACNSdashboard/Droppedtrafficbyreason15.png#lightbox)

- Utilize Heatmap for Source/Destination Pods: Review the Heatmap of Outgoing Drops for Top Source Pods or top destination pods to identify which pods are most affected. Brighter colours indicate higher rates of drops. Compare across time to detect patterns or spikes in specific pods.

[![Snapshots of Heatmap of Incoming Drops for top Destination pods](./media/advanced-container-networking-services/ACNSdashboard/heatmapofdroppaketsatdestinationpod16.png)](./media/advanced-container-networking-services/ACNSdashboard/heatmapofdroppaketsatdestinationpod16.png#lightbox)

- Investigate Stacked Outgoing Drops: Use the Stacked (Total) Outgoing/Incoming Drops by Source Pod chart to compare drop rates across affected pods. Identify if specific pods consistently show higher drops (e.g., kapinger-good-8468b88556-nf965 at 680 mp/s). Cross-reference these pods with their workloads, labels, and network policies to diagnose potential misconfigurations.

[![Snapshots of Total Outgoing Drops by source pods](./media/advanced-container-networking-services/ACNSdashboard/totalOutgoingdropsatdourcepods17.png)](./media/advanced-container-networking-services/ACNSdashboard/totalOutgoingdropsatdourcepods17.png#lightbox)
...
### Step2)	Investigate with Hubble Cli
...
[![Snapshots of Hubblecli logs for flows logs with verdict dropped](./media/advanced-container-networking-services/ACNSdashboard/hubbleCLIlogsfordropflow18.png)](./media/advanced-container-networking-services/ACNSdashboard/hubbleCLIlogsfordropflow18.png#lightbox)

use Hubble CLI to observe policy_denied flows, identify affected pods, and analyze blocked protocols or destinations. Inspect and validate network policies using kubectl, test connectivity with nc or nslookup, update policies to allow required traffic, and monitor drops for resolution.
...
[![Snapshots of Hubble UI for service dependency](./media/advanced-container-networking-services/ACNSdashboard/HubbleUIfordrops19.png)](./media/advanced-container-networking-services/ACNSdashboard/HubbleUIfordrops19.png#lightbox)
...
Identifying Network Bottlenecks: By analyzing the "Bytes Forwarded" and "Packets Forwarded" graphs, users can identify if there are any sudden drops or spikes indicating potential bottlenecks or congestion in the network.

Detecting Packet Loss: The "Packets Dropped" and "Bytes Dropped" sections help in identifying if there is significant packet loss occurring within specific clusters, which could indicate issues like faulty hardware or misconfigured network settings.
...
By comparing max and min ingress/egress bytes and packets, users can analyse performance trends and determine if certain times of day or specific workloads are causing performance degradation.

Diagnosing Drop Reasons: The "Bytes Dropped by Reason" and "Packets Dropped by Reason" sections help in understanding the specific reasons for packet drops, such as policy denials or unknown protocols. This information is crucial for addressing the root cause of network issues.

Node-Specific Analysis: The "Bytes Dropped by Node" and "Packets Dropped by Node" graphs provide insights into which nodes are experiencing the most packet drops. This helps in pinpointing problematic nodes and taking corrective actions to improve network performance.

Distribution of TCP connections: The graph here signifies the distribution of TCP connections across different states. For instance, if the graph shows an unusually high number of connections in the SYN_SENT state, it may indicate that the cluster nodes are having trouble establishing connections, due to network latency or misconfiguration. On the other hand, a considerable number of connections in the TIME_WAIT state could suggest that connections are not being properly released, potentially leading to resource exhaustion.
...
The second graph, titled "Heatmap of Dropped Outgoing HTTP Requests for Top 10 Source Pods in default," uses colour coding to represent the number of dropped requests. Darker colours indicate higher numbers of dropped requests, while.

lighter colours indicate fewer or no dropped requests. The alternating dark and light bands suggest periodic patterns in request drops.
...
Traffic Patterns and Volume: Analyze traffic trends to identify surges, drops, or imbalance in traffic distribution. Overloaded nodes or services may lead to bottlenecks or dropped requests.

Error Rates: Track trends in 4xx (invalid requests) and 5xx (backend failures) errors. Persistent errors indicate client misconfigurations or server-side resource constraints.

Dropped Requests: Investigate drops at specific pods or nodes. Drops often signal connectivity issues or policy-related denials.

Policy Enforcement and Configuration: Evaluate network policies, service discovery mechanisms, and load balancing settings for misconfigurations.

Heatmaps and Flow Metrics: Use visualizations like heatmaps to quickly identify error-heavy pods or traffic anomalies.

These metrics empower teams to diagnose and resolve networking issues, ensuring seamless service delivery and compliance with SLAs.
