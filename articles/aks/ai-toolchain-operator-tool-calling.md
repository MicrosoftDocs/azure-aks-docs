---
title: Configure tool calling with an AI inference service via the AI Toolchain Operator 
...
# Integrate Tool Calling with LLM Inference on Azure Kubernetes Service (AKS) with the AI toolchain operator add-on

In this article, you configure and deploy a KAITO inference workspace on Azure Kubernetes Service (AKS) with support for OpenAI-style tool calling. You also learn how to validate tool calling functionality using vLLM metrics and local function mocks.
...
It does this by invoking a defined “tool” with parameters it chooses based on the user’s request. Tool calling is highly useful for:
