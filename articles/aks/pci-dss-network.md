...
# Network security for an AKS regulated cluster for PCI DSS 4.0.1
...
When you're hosting a workload in Kubernetes, it's not sufficient to rely on traditional network constructs, such as firewall rules. PCI DSS 4.0.1 emphasizes the use of both cloud-native and Kubernetes-native controls for segmentation and secure communication. There are also Kubernetes-native constructs that control the flow of traffic within the cluster, such as the `NetworkPolicy` resource. We highly recommend that you refer to the Kubernetes documentation to understand the core concepts about isolated pods and ingress and egress policies.
...
You can integrate Azure Firewall with AKS and limit outbound traffic from the cluster, which is a key component of the CDE. The configuration is made easy with an AKS FQDN tag. The recommended process is provided in [Use Azure Firewall to protect Azure Kubernetes Service (AKS) deployments](/azure/firewall/protect-azure-kubernetes-service).
...
AKS optionally supports the ability to define an [HTTP proxy](/azure/aks/http-proxy), which you can use for additional outbound traffic control and monitoring for the cluster. The cluster nodes use the specified HTTP proxy configuration for routing outbound traffic. Also, a MutatingWebhook is registered to inject the proxy information into the pods at creation time, so it's recommended that workloads can inherit the same proxy information. Pods can override proxy information, so it's recommended to use an HTTP proxy in addition to an Azure Firewall.

AKS clusters should be created with the NetworkPolicy plugin. In AKS, [you have several options for your Network Policy plugin](/azure/aks/use-network-policies), including Calico, Azure Network Policy Management, and Cilium. With Calico network policy, you can either use Kubenet or Azure CNI. For Azure Network Policy Management, you can only use Azure CNI (not Kubenet). Both Azure and [Calico](https://www.tigera.io/project-calico/) Network Policy plugins are open source. For more information about Project Calico, see the [comprehensive PCI solution whitepaper](https://www.tigera.io/lp/kubernetes-pci-compliance/?utm_campaign=calicocloud&utm_medium=digital&utm_source=microsoft_aks_pciwhitepaper), which addresses many of the firewall requirements outlined in PCI DSS 4.0.1.

PCI DSS 4.0.1 expands requirements for network segmentation, risk-based validation of scope, and continuous review of firewall and router configurations. Use cloud-native networking security tools such as virtual networks, security groups, and micro-segmentation. Ensure containerized workloads use proper namespace isolation and secure communication protocols. Incorporate layered defense using Kubernetes network policies or similar tools in container environments.

#### Your responsibilities

|Requirement|Responsibility|
|---|---|
|[Requirement 1.1](#requirement-11)|Establish and implement firewall and router configuration standards, including risk-based validation of scope and regular review.|
...
The following image shows the network diagram of the reference implementation:

:::image type="content" source="media/pci-dss/network-topology-small.svg" alt-text="Diagram of the network topology." lightbox="media/pci-dss/network-topology-small.svg":::
...
The diagram should show how data flows to and from the workload and what information is passed from one resource to another. Make sure the diagram is kept current. Add a step to update the data flow diagram as part of the change management process.

Because this architecture is focused on the infrastructure and *not* the workload, we omitted illustrations here.
...
- NSGs secure traffic between the cluster and other components within the infrastructure. For example, in the reference implementation, there are NSGs on the subnet with node pools that block any SSH access attempts. Only traffic from within the virtual network is allowed.
...
You need to provide controls on the network flows and the components involved. The resulting infrastructure will have several network segments, each with many controls, and each control with a purpose. Make sure your documentation has breadth of coverage, from network planning to all configurations applied. It should also include details on ownership, with clear lines of responsibility and the roles that are responsible for them.

For example, know who's responsible for the governance of securing networks between Azure and the internet. In an enterprise, the IT team is usually responsible for configuration and maintenance of Azure Firewall rules, web application firewall (WAF) rules, NSGs, and other cross-network traffic. They might also be responsible for enterprise-wide virtual network and subnet allocation, and IP address planning.
...
The following table lists examples where you might allow traffic:
...
Have processes at least every six months to review the network configurations and the scoped rules. This ensures the security assurances are current and valid. Make sure you review the following configurations:
...
For information about private clusters, see [Create a private Azure Kubernetes Service (AKS) cluster](/azure/aks/private-clusters).
...
- Optionally, it's possible to use an HTTP proxy for monitoring and securing outbound (egress) traffic from the cluster to external resources.
   ...
   In addition to a firewall, some organizations might want to use an HTTP proxy to have additional controls on egress. We recommend that you still have the user-defined routes go to the firewall and to limit egress traffic to just go to the HTTP proxy. With this setup, if a pod tries to override the proxy, then the firewall can still block egress traffic.

   For more information, see [HTTP proxy support in Azure Kubernetes Service (AKS)](/azure/aks/http-proxy).
...
In addition to firewall rules and private networks, traffic flows are also secured through NSG rules. The following examples illustrate this architecture where the CDE is protected by denying traffic:
...
Install perimeter firewalls between all wireless networks and the cardholder data environment, and configure these firewalls to deny or permit only authorized traffic between the wireless environment and the cardholder data environment if traffic is necessary for business purposes.
...
Another critical system component is the API server that's used to do native Kubernetes tasks, such as maintain the state of the cluster and configuration. An advantage of using a private cluster is that API server endpoint isn't exposed by default. For information about private clusters, see [Create a private Azure Kubernetes Service (AKS) cluster](/azure/aks/private-clusters).
...
Review the following best practices for implementing a DMZ:
...
You can implement a perimeter network to protect AKS clusters. For more information, see [Cloud DMZ](/azure/cloud-adoption-framework/decision-guides/software-defined-network/cloud-dmz).
...
Don't  allow unauthorized outbound traffic from the cardholder data environment to the Internet.

##### Your responsibilities

You can block unauthorized outbound traffic using the following best practices:
...
Don't disclose private IP addresses and routing information to unauthorized parties.
...
Default settings provided by vendors must be changed. Default settings are common attack vectors and make the system prone to attacks. Consider the following best practices:
...
- Don't generate or provide SSH key access to nodes to administrator users. If emergency access is necessary, use the Azure recovery process to get just-in-time access.
...
For more information, see the [Microsoft cloud security benchmark](/security/benchmark/azure/introduction).
...
Application Gateway must not respond to requests on port 80. Don't perform redirects at the application level. This reference implementation has an NSG rule on that blocks port 80 traffic. The rule is on the subnet with Application Gateway.

If a workload in your cluster can't adhere to organizational policy around security compliance profiles or other controls (for example, limits and quotas), then make sure the exception is documented. You must monitor to ensure that only expected functionality is performed.
...
All administrative access to the cluster should be done by using the console. Don't expose the cluster's control plane.
...
All Azure resources used in the architecture must be tagged properly. The tags help in data classification and indicate whether the service is in-scope or out-of-scope. Meticulous tagging allows you to query for resources, keep an inventory, help track costs, and set alerts. Also maintain a snapshot of that documentation periodically.
