...
1. The Condition for the alert should be a Custom log search.
...
1. In the opened "Search query" box, paste one of the following custom queries and click "Review+Create" button.
...
1. Assign system assigned managed identity: while creating the alert, assign a managed identity so you can grant access to the necessary resources. To assign a managed identity during creation:
...
1. Make sure to assign the appropriate Reader roles.
...
1. In the **Search query** box, paste one of the following custom queries and then select the **Review+Create** button.
   ...
   The following query is for notifications of automatic upgrates for clusters:
   ...
   The following query is for notifications of automatic upgrates for Node OS:
...
To upgrade the cluster, wait for the automatic upgrader to start. Then verify that you promptly receive notices on the email configured to receive notices.
