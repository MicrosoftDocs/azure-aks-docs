...
- Azure Key Vault with 1) a firewall setting "allow public access from specific virtual networks and IP addresses" or 2) "disable public access" without AKS vnet integration enabled are not supported because it blocks traffic from the KMS plugin to the key vault.
