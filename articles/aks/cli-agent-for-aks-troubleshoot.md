...
- If you see requests retrying to `/chat/completions` in the responses, you're throttled by the token per minute (TPM) limits from the LLM. Increase the TPM limit or [apply for more quota](/azure/ai-foundry/openai/quotas-limit#request-quota-increases).
