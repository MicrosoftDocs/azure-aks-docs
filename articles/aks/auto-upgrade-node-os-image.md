...
|---|---|---|
| `Speed of shipping`| The typical build time for new VHD can be minimum 1 week, although in the event of CVEs faster rollouts periodically possible.The time for a new build to hit regions depends on safe deployment practices that can be monitored via [release-tracker]| SecurityPatch ships on a weekly cadence, slightly faster to build and release compared to nodeimage. It also employs safe deployment practices highlighted in [release-tracker]. SecurityPatch has the advantage of 'Live-patching' in Linux environments, where patching may not necessitate reimaging at times. |`SecurityPatch`|
