...
If you are using the [Istio-based service mesh add-on][istio-addon] and setting up istiod with a [Plugin Certificate Authority (CA)][istio-plugin-ca] or [secure ingress gateway][istio-secure-gateways], Azure Key Vault provider for Secrets Store CSI Driver is required for these features. Outbound network requirements for Azure Key Vault provider for Secrets Store CSI Driver can be found [here][akv-outbound].
