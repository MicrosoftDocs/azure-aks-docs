...
AWS WAF provides the AWS Managed Rules for AWS WAF, which consists of a set of managed rulesets maintained by AWS. These rulesets include the AWS Core Rule Set (CRS) and the AWS Managed Rules for Common Vulnerabilities and Exposures (CVEs). Users can easily enable these rulesets to enhance the security of their web applications. [Learn more about AWS WAF managed OWASP rulesets](https://aws.amazon.com/waf/features/managed-rules/). AWS WAF also supports custom rules, which enable users to define their own rule sets using the AWS WAF Rule Language. This allows for precise customization and protection against application-specific vulnerabilities and attack patterns. [Learn more about AWS WAF custom rules](https://docs.aws.amazon.com/waf/latest/developerguide/waf-creating-rules.html). AWS WAF offers advanced bot control features that allow users to set rules to block or rate-limit automated bot traffic. It provides options to differentiate between good and bad bots using string matching or machine learning algorithms. [Learn more about AWS WAF bot control](https://aws.amazon.com/waf/features/bot-control/).
