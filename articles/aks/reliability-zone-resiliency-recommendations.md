...
- The reason for the above recommendation is because
    ...
    - AKS uses best-effort zone balancing on the underlying Virtual Machine Scale Sets (VMSS) . However, in cases of zonal capacity constraints or zone-down scenarios, VMSS will retry provisioning in another zone. If allocation fails in more than one availability zone, VMSS will not create a new node in that node pool, causing the scale-up operation to fail. Cluster Autoscaler will then place the node pool in a [backoff](./cluster-autoscaler-overview.md#node-pool-in-backoff) state.
    - Also since the zone balancing of nodes is handled by underlying Virtual Machine Scale Sets there is no automatic rebalancing of running workloads 
    
- Using one node pool per availability zone helps overcome these limitations. Cluster Autoscaler treats each node pool as an independent entity, giving users greater flexibility to manage zone-specific scaling behavior
