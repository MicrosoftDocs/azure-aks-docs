...
> [!IMPORTANT]
> Node images older than 90 days can contain unpatched security vulnerabilities and may not work properly with recently released features. This may lead to scaling issues, node readiness issues as well as expose you to critical security vulnerabilities. In addition to this, depending on the age of the image version, it may place the cluster outside of the support scope until node image upgrades are performed.	AKS recommends that node pools be kept on a current image and that automatic upgrades/node image upgrades, be enabled if practical.
