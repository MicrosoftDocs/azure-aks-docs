...
For system node pools, VM sizes with fewer than two vCPUs and four GBs of RAM (memory) might not be used. To ensure that the required *kube-system* pods and your applications can reliably be scheduled, **[B series VMs][b-series-vm] are not supported for system node pools and [Av1 series VMs][a-series-vm] are not recommended**.
