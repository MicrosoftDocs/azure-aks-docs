...
> [!IMPORTANT]
> The DiskEncryptionSet we previously applied to the storage class will only encrypt new PVCs. Encrypting existing PVCs will require first detaching before using the Azure Disks API/CLI to update the underlying disks, as shown in [this related guide](https://learn.microsoft.com/en-us/azure/virtual-machines/linux/disks-enable-customer-managed-keys-cli#encrypt-existing-managed-disks).
