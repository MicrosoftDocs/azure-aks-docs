...
#### Option 3: Eviction Pod Autoscaler (preview)

Use the [Eviction Pod Autoscaler](eviction-pod-autoscaler.md) extension to proactively resolve PDB-blocked drains without bypassing PDB protections or requiring manual cleanup of quarantined nodes. The autoscaler detects when a PDB blocks eviction on a cordoned node and temporarily scales up the deployment's replicas so the disruption budget is satisfied. After the drain completes, it scales replicas back to their original count.

The Eviction Pod Autoscaler can also automatically create PDBs for deployments that don't have one, ensuring your workloads are protected during upgrade drains. For installation and configuration details, see [Use Eviction Pod Autoscaler to resolve PDB drain failures during AKS upgrades](eviction-pod-autoscaler.md).
...
* Use the [Eviction Pod Autoscaler](eviction-pod-autoscaler.md) to automate PDB creation and replica scaling during drain operations.
