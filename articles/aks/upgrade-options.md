...
## Validate LocalDNS and custom DNS before upgrading to Kubernetes 1.36

Starting with Kubernetes 1.36, LocalDNS can become active on an AKS Standard node pool that doesn't have an explicit LocalDNS profile. This changes the workload DNS path and can expose existing custom DNS or firewall configurations that support UDP port 53 but not TCP port 53.

Before upgrading a node pool to Kubernetes 1.36 or later:

1. Check whether the node pool has an explicit LocalDNS profile.
2. Test the custom VNet DNS server from an AKS node over both UDP and TCP:
...
3. Verify that NSGs, firewalls, NVAs, and routes permit both UDP and TCP port 53.
4. Upgrade a nonproduction node pool first and monitor CoreDNS and LocalDNS errors.
5. If the DNS path isn't ready for LocalDNS, explicitly configure `mode` as `Disabled` before upgrading.

For configuration and opt-out instructions, see [Configure LocalDNS in AKS](localdns-custom).
...
- **LocalDNS readiness:** For node pools upgrading to Kubernetes 1.36 or later, confirm that custom DNS servers and network controls support both TCP and UDP port 53, or explicitly disable LocalDNS.
