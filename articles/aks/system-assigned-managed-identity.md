---
title: Use a System-Assigned Managed Identity in Azure Kubernetes Service (AKS)
description: This article explains how to enable a system-assigned managed identity on a new or existing AKS cluster, get the principal ID of the system-assigned managed identity, and add a role assignment for the system-assigned managed identity.
author: davidsmatlak
...
# Customer intent: "As a DevOps engineer, I want to enable a system-assigned managed identity on my AKS cluster so that I can securely access Azure resources without managing credentials."
---

# Use a system-assigned managed identity in Azure Kubernetes Service (AKS)

This article explains how to enable a system-assigned managed identity on a new or existing AKS cluster, get the principal ID of the system-assigned managed identity, and add a role assignment for the system-assigned managed identity.
...
## Update an existing cluster to use a system-assigned managed identity
...
After you update the cluster to use a system-assigned managed identity instead of a service principal, the control plane and pods use the system-assigned managed identity for authorization when accessing other services in Azure. All nodes in your cluster will be reimaged so the Kubelet can immediately start using a managed identity as well. This reimage operation will cause downtime for your AKS cluster as the nodes in the node pools are cordoned, drained, and reimaged. Configure [Node Disruption Policy (preview)](./use-node-disruption-policy.md) to control when operations that require node reimage are allowed in your Azure Kubernetes Service (AKS) cluster.
...
> [!NOTE]
> For a virtual network (VNet), attached Azure disk, static IP address, or route table outside the default worker node resource group, you need to assign the `Network Contributor` role on the custom resource group.

Assign an Azure RBAC role to the system-assigned managed identity using the [`az role assignment create`][az-role-assignment-create] command. The following example assigns the **Network Contributor** role to the system-assigned managed identity. The role assignment is scoped to the resource group that contains the VNet.
...
## Create an AKS cluster with a system-assigned managed identity using Terraform

Add the following code to `main.tf` to create an AKS cluster with a system-assigned managed identity:
...
## Add a role assignment for a system-assigned managed identity using Terraform

Add the following code to `main.tf` to create a role assignment for the system-assigned managed identity. This example assigns the **Network Contributor** role to the system-assigned managed identity. The role assignment is scoped to the resource group that contains the VNet.
