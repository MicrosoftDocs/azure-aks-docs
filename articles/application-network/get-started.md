...
- For both existing and new AKS clusters, make sure they meet the following requirements. These requirements are validated when you join the cluster as a member:
    ...
    1. [Istio-based service mesh add-on](/azure/aks/istio-about) (`ServiceMeshProfile`) is **not** enabled. A cluster that already has the Istio add-on enabled can't join an Azure Kubernetes Application Network.
    ...
    1. [Managed Kubernetes Gateway API installation](/azure/aks/managed-gateway-api) is enabled.
...
1. [Istio-based service mesh add-on](/azure/aks/istio-about) (`ServiceMeshProfile`) is **not** enabled.
1. [AKS-managed Microsoft Entra integration](/azure/aks/entra-id-control-plane-authentication) is enabled.
1. [OIDC issuer](/azure/aks/use-oidc-issuer) is enabled.
1. [Managed Kubernetes Gateway API installation](/azure/aks/managed-gateway-api) is enabled.
