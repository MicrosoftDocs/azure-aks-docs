...
Fleet Manager provides [Approval Gates](./update-strategies-gates-approvals.md) in update runs so administrators have additional controls over when scopes in an update run are processed.

Events published to Event Grid by Approval Gates can be used for notifications (i.e. email) or automation integration (i.e. custom health checks). 
