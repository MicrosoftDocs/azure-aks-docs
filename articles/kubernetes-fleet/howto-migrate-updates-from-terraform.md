...
You can choose either the Stable or Rapid Kubernetes release channels to automatically increment the Kubernetes minor when a new minor is released, or if you want only patch releases for a minor you can use the [Target Kubernetes minor version channel](./update-automation.md#target-kubernetes-minor-version-updates-preview) which is currently in preview.
