...
- **Azure Resource Group for hub cluster resources:** like any AKS cluster, the hub cluster has Azure resources such as an agent node pool VMSS and virtual network which are created in a managed resource group with the naming format `MC_FL_{fleet_resource_group}_{fleet_manager_name}_{azure_region}`.
